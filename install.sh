...
echo "  ℹ️  SERVER_MODE=true: to use Claude Code memory on this machine (optional),"
echo "     register yourself after starting the service, then update your API key:"
echo "       curl -X POST http://localhost:1995/api/v1/users/register \\"
echo "            -H 'Content-Type: application/json' \\"
echo "            -d '{\"user_id\": \"your_id\", \"zerog_wallet_key\": \"your_wallet_key\"}'"
echo "     Then add to ~/.claude/settings.json (env section):"
echo "       EVERMEMOS_API_KEY  — the api_key from the response above"
echo "       EVERMEMOS_USER_ID  — your user_id"
fi
echo "============================================================"
echo ""
